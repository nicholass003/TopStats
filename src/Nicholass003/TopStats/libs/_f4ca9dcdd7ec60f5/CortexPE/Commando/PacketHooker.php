...
namespace Nicholass003\TopStats\libs\_f4ca9dcdd7ec60f5\CortexPE\Commando;

use Nicholass003\TopStats\libs\_f4ca9dcdd7ec60f5\CortexPE\Commando\exception\HookAlreadyRegistered;
use Nicholass003\TopStats\libs\_f4ca9dcdd7ec60f5\CortexPE\Commando\store\SoftEnumStore;
use Nicholass003\TopStats\libs\_f4ca9dcdd7ec60f5\CortexPE\Commando\traits\IArgumentable;
use Nicholass003\TopStats\libs\_f4ca9dcdd7ec60f5\muqsit\simplepackethandler\SimplePacketHandler;
...
class PacketHooker implements Listener {
	...
	private static function generateOverloads(CommandSender $cs, BaseCommand $command): array {
		...
		foreach($command->getSubCommands() as $label => $subCommand) {
			...
			$scParam = CommandParameter::enum($label, new CommandHardEnum($label, [$label]), 0);
